...
public class SpookyTimeSmoothMode extends AngleSmoothMode {
    ...
    public Angle limitAngleChange(Angle currentAngle, Angle targetAngle, Vec3d vec3d, Entity entity) {
        ...
        float yaw = canAttack ? 0 : (float) (randomLerp(4, 6) * Math.sin(System.currentTimeMillis() / 44D));
        float pitch = canAttack ? 0 : (float) (randomLerp(2, 3) * Math.cos(System.currentTimeMillis() / 44D));
        ...
        float lineYaw = (Math.abs(yawDelta / rotationDifference) * 179);
        float linePitch = (Math.abs(pitchDelta / rotationDifference) * 179);

        float moveYaw = MathHelper.clamp(yawDelta, -lineYaw, lineYaw);
        float movePitch = MathHelper.clamp(pitchDelta, -linePitch, linePitch);

        Angle moveAngle = new Angle(currentAngle.getYaw(), currentAngle.getPitch());
        moveAngle.setYaw(MathHelper.lerp(randomLerp(speed, speed + 0.32F), currentAngle.getYaw(), currentAngle.getYaw() + moveYaw) + yaw);
        moveAngle.setPitch(MathHelper.lerp(randomLerp(speed, speed + 0.35F), currentAngle.getPitch(), currentAngle.getPitch() + movePitch) + pitch);
        return moveAngle;
    }
